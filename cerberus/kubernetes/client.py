...
def get_node_info(node):
    try:
        return cli.read_node_status(node, pretty=True, limit=request_chunk_size)
    except ApiException as e:
...
def get_pod_status(pod, namespace):
    try:
        return cli.read_namespaced_pod_status(pod, namespace, pretty=True, limit=request_chunk_size)
    except ApiException as e:
...
def check_master_taint(master_nodes, master_label):
    ...
    all_master_info = get_all_nodes_info()
