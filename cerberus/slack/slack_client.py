...
def initialize_slack_client():
    try:
        ...
        logging.error("Couldn't create slack WebClient. Check if slack env " "varaibles are set. Exception: %s" % (e))
...
def post_message_in_slack(slack_message, thread_ts=None):
    slack_client.chat_postMessage(channel=slack_channel_ID, link_names=True, text=slack_message, thread_ts=thread_ts)


# Get members of a channel
def get_channel_members():
    return slack_client.conversations_members(channel=slack_channel_ID)
...
def slack_report_cerberus_start(cluster_info, weekday, watcher_slack_member_ID):
    response = slack_client.chat_postMessage(
        ...
        link_names=True,
        text="%s Cerberus has started monitoring! " ":skull_and_crossbones: %s" % (slack_tag, cluster_info),
    )  # noqa
    global thread_ts
